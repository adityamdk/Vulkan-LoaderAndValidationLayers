...
//TODO remove these maps and use the wrapped objes instead
...
static void create_instance_register_extensions(const VkInstanceCreateInfo *pCreateInfo, VkInstance instance) {
    uint32_t i;
    ...
    instanceExtMap[pDisp].wsi_surface_enabled = false;
    instanceExtMap[pDisp].wsi_surf_xcb_enabled = false;
    instanceExtMap[pDisp].wsi_surf_xlib_enabled = false;
    instanceExtMap[pDisp].wsi_surf_mir_enabled = false;
    instanceExtMap[pDisp].wsi_surf_wayland_enabled = false;
    instanceExtMap[pDisp].wsi_surf_win32_enabled = false;
    instanceExtMap[pDisp].debug_report_enabled = false;
    for (i = 0; i < pCreateInfo->enabledExtensionCount; i++) {
        if (strcmp(pCreateInfo->ppEnabledExtensionNames[i], VK_KHR_SURFACE_EXTENSION_NAME) == 0)
            instanceExtMap[pDisp].wsi_surface_enabled = true;
        if (strcmp(pCreateInfo->ppEnabledExtensionNames[i], VK_EXT_DEBUG_REPORT_EXTENSION_NAME) == 0)
            instanceExtMap[pDisp].debug_report_enabled = true;
#ifdef VK_USE_PLATFORM_XCB_KHR
        if (strcmp(pCreateInfo->ppEnabledExtensionNames[i], VK_KHR_XCB_SURFACE_EXTENSION_NAME) == 0)
            instanceExtMap[pDisp].wsi_surf_xcb_enabled = true;
#endif  // VK_USE_PLATFORM_XCB_KHR
#ifdef VK_USE_PLATFORM_XLIB_KHR
        if (strcmp(pCreateInfo->ppEnabledExtensionNames[i], VK_KHR_XLIB_SURFACE_EXTENSION_NAME) == 0)
            instanceExtMap[pDisp].wsi_surf_xlib_enabled = true;

#endif  // VK_USE_PLATFORM_XLIB_KHR
#ifdef VK_USE_PLATFORM_WAYLAND_KHR
        if (strcmp(pCreateInfo->ppEnabledExtensionNames[i], VK_KHR_WAYLAND_SURFACE_EXTENSION_NAME) == 0)
            instanceExtMap[pDisp].wsi_surf_wayland_enabled = true;

#endif  // VK_USE_PLATFORM_WAYLAND_KHR
#ifdef VK_USE_PLATFORM_MIR_KHR
        if (strcmp(pCreateInfo->ppEnabledExtensionNames[i], VK_KHR_MIR_SURFACE_EXTENSION_NAME) == 0)
            instanceExtMap[pDisp].wsi_surf_mir_enabled = true;

#endif  // VK_USE_PLATFORM_MIR_KHR
#ifdef VK_USE_PLATFORM_WIN32_KHR
        if (strcmp(pCreateInfo->ppEnabledExtensionNames[i], VK_KHR_WIN32_SURFACE_EXTENSION_NAME) == 0)
            instanceExtMap[pDisp].wsi_surf_win32_enabled = true;
#endif  // VK_USE_PLATFORM_WIN32_KHR
